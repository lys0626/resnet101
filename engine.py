...
class Engine(object):
    def __init__(self, args):
        ...
        # --- 2. 使用新格式命名 (保留时间戳以防覆盖) ---
        # (self.args.data_set 来自 main.py)
        # (self.args.start_time 来自 utils.py)
...
# 如果不是 DDP 模式 (args.distributed == False), 
# self.model 已经通过 utils.init 被正确放置在单 GPU 上，
# 并且不需要封装。
    ...
    def on_end_batch(self, outputs, targets_gt, loss, image_name=''):
        ...
        if self.args.distributed:
            ...
            # 在单 GPU 模式下, 不需要 'concat', loss_all 就是 loss
    ...
    def save_checkpoint(self, is_train, is_best):
        ...
        if is_best:
            ...
            # --- 2. 使用新格式命名 ---
            # (self.args.data_set 来自 main.py)
            file_best = f'{self.args.data_set}_{method_name}.pt'
